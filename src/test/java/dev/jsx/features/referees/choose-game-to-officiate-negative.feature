Feature: referee can choose an upcoming game to officiate
  Scenario: referee chooses an invalid upcoming game to officiate
    ...
    When  the referee enters a non-existing game ID into the Game ID input
    And   the referee clicks on the Submit button
    Then  an alert should appear saying "Application Unsuccessful"